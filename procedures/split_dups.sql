-- Split duplicates from a table into a temporary table
-- Common Usage: select split_dups('table_name', 'id, columns, by, comman', 'output_table_name')
-- Note: Use 'schema.table_name' for permanent tables in a specific schema

drop procedure public.split_dups(text, text, text);

CREATE procedure public.split_dups(
...
BEGIN
    ...
    query1 := '
        create temporary table ' || quote_ident(output_table) || ' as
        select *
        from ' || source_table || '
        where 1=0;
    ';
...
GRANT execute ON PROCEDURE public.split_dups(text, text, text) TO PUBLIC;

-- Examples

-- drop table if exists alphabet_split;
-- call public.split_dups('public.alphabet', 'letter', 'alphabet_split');
-- select * from alphabet_split; -- this should not exist

-- drop table if exists alphabet_split;
-- call public.split_dups('public.alphabet_dups', 'letter', 'alphabet_split');
